...
// import { getParticipants, promoteParticipants, registerThroughCSV, toggleCourseStatus } from "../../redux/admin/adminActions";
import { Link, Route } from "react-router-dom";
import Statistics from "./Statistics";
import Participant from "./Student";
...
const Course = (props) => {
    ...
    const course = useSelector(
        (state) => state.admin.data.courses.filter((course) => course._id === courseID)[0]
    );
    ...
    const CSVSubmitHandler = () => {
        ...
        // dispatch(registerThroughCSV({ CSVParticipants, isTeamCourse: course.isTeamCourse, courseID: course._id }, handleCSVSubmitSuccess, handleCSVError))
        // dispatch(getParticipants(course._id));
        setLoading(false)
    ...
    // console.log(admin.data.courses, "ertghjjhjknm,")

    // console.log(course.teamLeaders, "teamLeaders")



    // console.log(course, "ertghjjhjknm,")
    ...
    //     dispatch(getParticipants(course._id));
    //     setLoading(false)
    // }, [loading, dispatch, course._id ]);

    const rowSelection = {
        onChange: async (selectedRowKeys, selectedRows) => {
            // console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
            if (course.teamLeaders.length !== 0 && course.teamLeaders !== undefined && course.teamLeaders !== null) {
    ...
    const handleSuccess = () => {
        ...
        setTimeout(() => {
            hist.push(`/admin-p/admin-dashboard/${course._id}`);
        }, 2010);
        // dispatch(getParticipants(course._id));
    }
    ...
    const handleSubmit = () => {
        ...
        if (maxRound === course.numOfRounds) {
            ...
            // dispatch(promoteParticipants(selectionState.selectedIDs, maxRound, courseID, handleSuccess, handleError))
        }
        setSelectionState({
            selectedIDs: [], // Check here to configure the default column
            numOfTeams: 0
    ...
    const handleToggle = () => {
        // console.log("toggle")
        // dispatch(toggleCourseStatus(courseID, handleToggleSuccess, handleError))
    }
    return (

        <Fragment >
            <Route key={course._id} exact path={`/admin-p/admin-dashboard/${course._id}`} >
                ...
                <Row style={{ padding: 10, paddingTop: 20 }}>
                    <Col span={15} >
                        <Title level={3} >
                            {" "}
                            {course.displayName}  {course.isTeamCourse ? <Tag color="#f50">Team Course</Tag> : <Tag color="#87d068">Solo Course</Tag>}
                        </Title>
                    </Col>
                    {
                        admin.isEvaluator ?
                            ...
                            : <Col>
                                <Row gutter={10}>
                                    ...
                                    <Col span={4}>
                                        <Switch checkedChildren="Live" unCheckedChildren="Not live" defaultChecked={course.isLive} onChange={handleToggle} />
                                    </Col>
                ...
                <Spin spinning={mainLoading}>
                    <Tabs defaultActiveKey="1">
                                ...
                                <Col span={24}>
                                    {
                                        course.participants.length ?
                                            <Statistics courseID={courseID} /> : null
                                    }
                        ...
                        {
                            (course.isTeamCourse) ?
                                course.roundWiseSelectedTeamLeaders ? course.roundWiseSelectedTeamLeaders.map((thisRoundSelectedTeamLeaders) => {
                                    // console.log(course.isTeamCourse, "course.teamLeaders")
                                    i++;
                                    if (i <= course.numOfRounds) {
                                        return (
                                            <TabPane tab={`Round ${i}`} key={i + 1}>
                                                ...
                                                {
                                                    (thisRoundSelectedTeamLeaders[i - 1] !== undefined) ? console.log(Object.keys(thisRoundSelectedTeamLeaders[i - 1]).filter((key) => key !== "courseID" || key !== "userID").map(key => {
                                                        if (key === "pdfSubmissionUrls") {
                                                            ...
                                                            return {
                                                                ...
                                                                render: (text, teamLeader) => (
                                                                    <Space size="middle">
                                                                        <Link to={`/esummmit-admin/dashboard/${course._id}/${teamLeader._id}`}>View Submissions</Link>
                                                                    </Space>
                                                                )
                                                            }
                                                        }
                                                            ...
                                                            expandable={{
                                                                expandedRowRender: (teamLeader => {
                                                                    if (admin.isEvaluator) {
                                                                        return <Submissions submissions={teamLeader.submissions} course={course} />
                                                                    }
                                                                    else {
                                                                        return <TeamLeader teamLeader={teamLeader} course={course} />
                                                                    }
                                                                            ...
                                                                            <Col >
                                                                                {
                                                                                    thisRoundSelectedTeamLeaders.length ?
                                                                                        (<Button type="primary"  >
                                                                                            <CSVLink
                                                                                                filename={`E-Cell_${course.name}_round${i}_data.csv`}
                                                                                                data={
                                                                                                    course.participants.filter(participant => participant.round === i).map(participant => {
                                                                                                        j++
                                ...
                                course.roundWiseSelectedParticipants ? course.roundWiseSelectedParticipants.map((thisRoundSelectedParticipants) => {
                                    // console.log(course.teamLeaders, "course.teamLeaders")
                                    i++;
                                    if (i <= course.numOfRounds) {
                                        return (
                                            <TabPane tab={`Round ${i}`} key={i + 1}>
 ...
 */}
                                                {/* {
                                                (thisRoundSelectedParticipants[i - 1] !== undefined) ? console.log(Object.keys(thisRoundSelectedParticipants[i - 1]).filter((key) => key !== "courseID" || key !== "userID").map(key => {
                                                    if (key === "pdfSubmissionUrls") {
                                                        ...
                                                        return {
                                                            ...
                                                            render: (text, participant) => (
                                                                <Space size="middle">
                                                                    <Link to={`/esummmit-admin/dashboard/${course._id}/${participant._id}`}>View Submissions</Link>
                                                                </Space>
                                                            )
                                                        }
                                                    }
                                                ...
                                                { thisRoundSelectedParticipants !== undefined ?
                                                    <div>
                                                        ...
                                                        <Table loading={loading}
                                                            ...
                                                            //         return <Participant participant={participant} course={course} />
                                                            //     }
                                                            //     ),
                                                            // }}
                                                            size="default" />
